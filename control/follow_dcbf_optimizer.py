...
class NmpcDcbfOptimizerParam:
    def __init__(self):
        ...
        self.distance_between_humandog = 0.8
...
class NmpcDbcfOptimizer:
    ...
    def add_follow_huamn_cost(self, huamn_states, param):
        self.costs["close2human"] = 0
        for i in range(param.horizon - 1):
            x_diff = self.variables["x"][0:2, i] - huamn_states[i, 0:2]
            t_diff = self.variables["x"][2, i] - huamn_states[i,2] - np.pi
            self.costs["close2human"] +=  param.dis_Q * (ca.norm_2(x_diff) - param.distance_between_humandog) ** 2
            self.costs["close2human"] +=  param.dis_T * t_diff**2 

    

        # x_diff = self.variables["x"][:, -1] - huamn_states[-1, :]
        # self.costs["close2human"] += param.terminal_weight * ca.mtimes(x_diff.T, ca.mtimes(param.mat_Q, x_diff))
    ...
    def setup(self, param, system, huamn_states, obstacles):
        ...
        self.add_follow_huamn_cost(huamn_states, param)
