import numpy as np
from models.guide_dog import GuideDogDynamics

class GuideDogTrajectoryGenerator:
    ...
    def generate_trajectory_internal(self, pos, global_path):
        ...
        path_dog_head = np.array([[np.arctan2((path_dog[i+1][1]-path_dog[i][1]), (path_dog[i+1][0]-path_dog[i][0]) )] for i in range(len(path_dog)-1) ])
        path_dog_head = np.vstack([path_dog_head,np.array([0.0])])
